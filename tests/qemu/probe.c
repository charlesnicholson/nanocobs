...
static int run(void) {
  ...
  for (unsigned n = 0; n <= PAYLOAD_MAX; ++n) {
    ...
    for (unsigned long m = 0; m < combos; ++m) {
...
#endif
      if ((ea != eb) || (la != lb) || memcmp(a, b, sizeof a)) {
        fail("encode", m, n);
        continue;
      }
      ...
      memset(ra, 0xCD, sizeof ra);
      memset(rb, 0xCD, sizeof rb);
      size_t da = 0, db = 0;
      cobs_ret_t const dra = cobs_ref_decode(a, la, ra, sizeof ra, &da);
      cobs_ret_t const drb = cobs_decode(b, lb, rb, sizeof rb, &db);
      if ((dra != drb) || (da != db) || memcmp(ra, rb, sizeof ra)) {
        fail("decode", m, n);
        continue;
      }
      if ((dra != COBS_RET_SUCCESS) || (da != n) || (n && memcmp(ra, dec, n))) {
        fail("roundtrip", m, n);
        continue;
      }
      ++passed;
