...
`latest`. The npm publish authenticates by OIDC through npm's trusted publisher for this
repository, so there is no token to rotate. Both publishes skip a version that is already
